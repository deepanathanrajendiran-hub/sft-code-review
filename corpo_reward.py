...
# v5.1 rebalance (beta=2.0, clean penalty 0.25) recovered recall from v5.0's collapse but
# the local 632-record analysis (2026-05-29) showed it just slid the OTHER way: on the 275
# records where v5.1 and v4 disagree it FABRICATED concrete bugs 11x vs 3 real catches in 45
# samples, because beta=2 weighted recall 4x precision -> over-flagging was reward-positive.
# v5.2: dial beta 2.0->1.5 (recall favored ~2.25x, not 4x) and steepen clean penalty 0.25->0.35
# so asserting unsupported defects costs more. Goal: convert v5.1's extra assertiveness into
# real catches instead of fabrications. NOTE: v4 sits near its precision/recall frontier, so RL
# tuning may only pick a different frontier point — the higher-confidence lever is v4.1 DATA.
RECALL_BETA = 1.5          # F-beta: recall weighted beta^2=2.25x precision on labeled records
